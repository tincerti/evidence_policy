...
Our ideal ballot measure would involve a policy with a scientific or policy research consensus on its overall merit or demerit. This would likely represent a technical subject on which people have relatively weak priors. Policy issues such as health care or housing could work very well for our purposes. A list of 2020 California ballot measures or those approved by yet to meet the minimum signature threshold eligible or requiring additional is provided in \autoref{tab: initiatives}.
...
We intend to use clustered random assignment rather than individual-level randomization, i.e assigning voting precincts to treatment and control rather than individual nested within them. Following \citet{arceneaux2005using}'s advice, we argue that such an approach is not only easier and potentially less expensive to implement, it also opens the door to clearer analyses without the need for survey data (information on individual voting decisions is not publicly available).
To compensate for the loss of statistical power involved with the use of clusters, we will use covariates such as past voting behaviours, age, gender and party registration (and all other variables available in the voters files that are in any way predictive of one's vote on a ballot initiative).
...
Conducting a survey experiment in complement of the field experiment will provide at least two benefits: (i) substantially, explore some potential mechanisms and (ii) methods wise, contribute to the growing literature on the replicability of survey experiments results in the field. 

Substantially, a survey experiment allows us to add a set of outcome variables: in addition to asking participants whether they would vote yes or no on the issue, we could ask them basic factual questions on the policy topic. If we find differences in our primary outcome (voting decision), adding this second set of outcomes could help us test whether this difference in voting can be partly explained by a difference in the understanding of the issue. 
\citet{knobloch2014empowering} study the impact of a Citizen Assembly in Oregon and the presence of a synthesis of the Assembly in the voters pamphlets. A state-wide survey shows that voters seemed to read and appreciate the synthesis, and they conduct a survey experiment to show that the synthesis lead to higher knowledge level on the issue than other sources of information (interest group, neutral informational by government). But they do not study vote as the outcome variable of interest, and do not have a field experiment. 
\bigbreak
A second reason to do a survey experiment is to contribute to a methods question. A growing literature suggests that survey experimental results often do not replicate in the field. By contrast, we hypothesize that survey experiments should better approximate the context of ballot initiatives as: (1) the vote choices are real ballot propositions rather than hypothetical candidates, and (2) partisan cues are less prevalent in (technical) ballot proposals. The ballot initiative setting would allow us to design the survey so that it exactly replicates the field setting, with the only differences being hypothetical vote choice versus actual.
